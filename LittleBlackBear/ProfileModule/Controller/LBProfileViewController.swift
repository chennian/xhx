...
class LBProfileViewController: UITableViewController {
    ...
    private func setupUI() {
        ...
        headerView.uploadIconImageAction={[weak self](imageView) in
            ...
            strongSelf.showPhotoPickerView(finishHandel: { (image) in
                ...
                manager.uploadImg(objectKey: objecKey, image: image.compressImage(image: image.compressImage(image: image, maxLength: 0)!, maxLength: 0)!,imageName:"userIcon",bucketName: BucketName, endPoint: EndPoint,path:path){[weak self] (success) in
                    if success{
    ...
    fileprivate func merchantStatus() {
        ...
        listControllers = [[NSStringFromClass(LBMyWalletWebViewController.self),
                            ...
                            NSStringFromClass(LBMyNewsViewController.self),
        ...
        if isMerc == TRUE,isAgent == FALSE {
            ...
            listControllers.insert([
                ...
                NSStringFromClass(LBPayQRCodeViewController.self),
                NSStringFromClass(LBMyAcountBookWebViewController.self),
                NSStringFromClass(LBAddSeckillController.self),
                NSStringFromClass(LBFansWebViewController.self),
