    ...
    override func imageCell(_ imageCell: ImageTableViewCell, imageButtonClick imageButton: UIImageView) {
        ...
        showPhotoPickerView { [weak self] (image) in
            ...
            if self?.applyType == .company {
                ...
                manager.uploadImg(objectKey: objecKey, image: image.compressImage(image: image, maxLength: 0)!,imageName:"licenseImage",bucketName: BucketName, endPoint: EndPoint,path:path){[weak self] (success) in
                    if success{
                ...
                manager.uploadImg(objectKey: objecKey, image: image.compressImage(image: image, maxLength: 0)!,imageName:imgNames[tag],bucketName: BucketName, endPoint: EndPoint,path:path){[weak self] (success) in
                    if success{
                        guard path.count > 0 else{return}
                        switch tag {
                        case 0:
