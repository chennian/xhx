//
//  BXIndustryDetailPickView.swift
//  BaiXiangPay
//
...
class LBIndustryDetailPickView: UIView {
	
	var dissMissIndustryPickerViewAction: (()->())?
	var didSelectRowWithValue:((_ id:Int,_ value2:String)->())?
	var List = [LBJSON]()
...
extension LBIndustryDetailPickView:UIPickerViewDelegate,UIPickerViewDataSource{
	...
	func pickerView(_ pickerView: UIPickerView, titleForRow row: Int, forComponent component: Int) -> String? {
		if List.count > 0 && row <= List.count {
			...
			if (self.didSelectRowWithValue != nil) {
				self.didSelectRowWithValue!( dictionary["id"].intValue, dictionary["name"].stringValue)
			}
