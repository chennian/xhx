...
public class PhotoBrowser: UIViewController {
    ...
    /// 容器layout
     fileprivate lazy var flowLayout: PhotoBrowserLayout = {
    ...
    /// PageControl
     fileprivate lazy var pageControl: UIView? = { [unowned self] in
...
extension PhotoBrowser: UICollectionViewDataSource {
    ...
    public func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
     ...
     fileprivate func imageFor(index: Int) -> (UIImage?, highQualityUrl: URL?, rawUrl: URL?) {
        guard let delegate = photoBrowserDelegate else {
            return (nil, nil, nil)
        }
        // 缩略图
