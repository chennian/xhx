...
public class PhotoBrowserCell: UICollectionViewCell {
    ...
    /// 查看原图按钮
     fileprivate lazy var rawImageButton: UIButton = { [unowned self] in
    ...
    /// 取图片适屏frame
     fileprivate var fitFrame: CGRect {
        let size = fitSize
    ...
    /// 记录pan手势开始时，手势位置
     ...
     fileprivate var shouldLayout = true
    ...
    /// 加载图片
     fileprivate func loadImage(withPlaceholder placeholder: UIImage?, url: URL?) {
    ...
    /// 根据url从缓存取图像
     fileprivate func imageFor(url: URL?) -> UIImage? {
