...
class PhotoPreviewCell: UICollectionViewCell, UIScrollViewDelegate {
    ...
    var imgRUL : String?{
        didSet{
            imageView.kf.setImage(with: URL(string : imgRUL!), placeholder: nil, options: nil, progressBlock: nil) { (_, error, type, _) in
                if error == nil{
    ...
    func resizeImageView() {
        ...
        self.imageView.frame = CGRect(x: self.imageContainerView.x, y: self.imageContainerView.x - LL_StatusBarExtraHeight, width: self.imageContainerView.width, height: self.imageContainerView.height)
