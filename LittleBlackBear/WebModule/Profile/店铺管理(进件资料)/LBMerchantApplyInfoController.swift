//
//  LBMerchantApplyInfoController.swift
//  LittleBlackBear
//
...
class LBMerchantApplyInfoController: UIViewController {

    fileprivate let tableView:UITableView = UITableView().then{
        $0.backgroundColor = color_bg_gray_f5
        $0.register(LBMerchantAppleFieldCell.self)
...
extension LBMerchantApplyInfoController:UITableViewDelegate,UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.row == 0 {
            let cell:LBMerchantAppleFieldCell = tableView.dequeueReusableCell(forIndexPath: indexPath)
            return cell
