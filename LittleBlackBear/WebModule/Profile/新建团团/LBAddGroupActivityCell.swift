...
class LBAddGroupActivityCell: SNBaseTableViewCell {

    var imgTap2 =  PublishSubject<(AliOssTransferProtocol,String)>()
    ...
    func bindEvent(){
        ...
        activityImge.rx.controlEvent(UIControlEvents.touchUpInside).asObservable().subscribe(onNext: {[unowned self] () in
            self.imgTap2.onNext((self.activityImge,self.activityImge.fuName))
