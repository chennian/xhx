//
//  LBForgotPasswordViewController.swift
//  LittleBlackBear
//
...
class LBForgotPasswordViewController: LBRegistLoginBaseViewController {
    ...
    override func viewDidLoad() {
        ...
        cellItems = [
            .input(imgName: "userLogoIcon", placeHolder: "请填写注册手机号"),
        ...
        textFieldKeyBoradTys=[.phone,.number,.password]
    ...
    func textFieldDidEditing(_ textField:UITextField){
        if  textField.placeholder == "请输入验证码" {
            if (textField.text ?? "" ).count >= 6{
                guard let text = textField.text else{return}
                textField.text = text.substring(to: text.index(text.startIndex, offsetBy: 6))
            }
        }else if textField.placeholder == "请填写注册手机号" {
            if (textField.text ?? "" ).count >= 11{
                guard let text = textField.text else{return}
                textField.text = text.substring(to: text.index(text.startIndex, offsetBy: 11))
            }
        }else if textField.placeholder == "请填写新密码" {
    ...
    override func registLoginCell(registLogin cell: LBRegistLoginCell, verificationButtonClick verificationButton: SMSButton) {
        ...
        LBSendRegValiMsgCodeServer.sendRegValiMsgCode(phone: phone, type: "1")
    }
